...
참고로 p\[1]+2 만 쓰면 값이 아닌 주소가 출력되기에 \* 를 붙여줘야만 한다.\
그런데 왜 **p\[0]\[1]** 는 값이고, **p\[1]+2** 는 주소가 되는 걸까?\
\
p\[0], p\[1] -> 둘 다 주소를 가리키고 있으므로 이때는 아직 값이 아니다.\
그러나 내부적으로 컴파일러는 a\[b] 를 \*(a+b) 로 계산한다.

즉, p\[0]\[1] 은 먼저 p\[0]  을 계산하고 -> (p\[0])\[1] 를 계산하므로
